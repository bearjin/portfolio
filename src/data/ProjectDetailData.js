const projectDetailData = {
    ...
    Hanwha: {
        ...
        img: 'hanwha',
        txt: [
            `리뉴얼된 홈페이지는 모바일과 PC 등 다양한 환경에서 최적화된 크기와 디자인으로 모든 콘텐츠를 이용할 수 있습니다. 
            또한 직관적이고 편리한 UI(사용자 인터페이스)를 적용해 방문자들이 많이 찾는 메뉴들을 쉽게 찾을 수 있도록 했습니다.`,
            `메인페이지에서 YouTube API를 사용해 영상을 컨트롤 하였고, 슬라이드는 Swiper API 를 사용해서 제작했습니다. 
    ...
    Kelim: {
        ...
        img: 'kelim',
        txt: [
            `계림요업 오피셜 사이트는 국내 최고 위생도기 기업으로서의 기업 이미지를 강화하고 자연과 사람이 함께 공존하며 성장해 가는 기업이미지를 강화하고자 노력했습니다.
            통합 제품 정보 뿐만 아니라 다양한 CS 채널 등을 제공함으로써, 세일즈에도 도움이 될 수 있는 사이트로 개발했습니다.
            `,
    ...
    Daewoo: {
        ...
        img: 'daewoo',
        txt: [
            `대우건설 홈페이지는 신뢰감 있는 기업 이미지를 전달하기 위해 대우건설의 뉴비전과 실적 영상을 메인에 활용하였으며, 일관된 톤 앤 매너, 안정적인 레이아웃과 가독성으로 사용성을 강화시켰습니다.`,
            `전·현직 임직원들의 소통 창구인 OB사랑방을 담당해 반응형 웹으로 구축하여 다양한 사용자 환경을 고려했습니다. 
            로그인, 회원가입 등의 폼요소들에 대한 코딩을 진행하며 구축 시 주의해야될 점들을 새롭게 배우며 HTML,CSS의 기본기들을 다질수 있었습니다. 
            사업분야 페이지의 원페이지 기능을 제작하며 스크롤과 트랜스폼,트랜지션을 활용한 시차모션들을 많이 배울 수 있었습니다.
            `
        ],
    ...
    Jaseng: {
        ...
        img: 'jaseng',
        txt: [
            `자생의료재단의 규모감과 메세지를 효과적으로 전달하기 위해 기존의 정적인 콘텐츠를 동적이고 시각적인 콘텐츠로 개선했습니다.`,
            `처음으로 구축 프로젝트에 참여하여 어렵지 않은 서브페이지들 위주로 코딩 작업을 진행했고 구축 업무가 어떻게 이루어지는지에 대해 전반적으로 하나하나 배워 나갈수 있었습니다. 
            SNS 연동, 프린팅 기능 등등 이전에 접해보지 못한 새로운 기능들을 많이 접해 볼 수 있었습니다.
            `
        ],
